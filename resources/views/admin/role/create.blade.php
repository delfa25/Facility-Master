    ...
    <div class="flex-1 flex flex-col pl-6 pr-4 py-2">
        ...
        <main class="flex-1 p-6 items-center overflow-auto">
            <div class="max-w-xl ml-[250px]">
                <h1 class="text-2xl font-bold mb-6">Créer un profil</h1>
                <div class="bg-white rounded-lg shadow p-6">
                    <form action="{{ route('roles.store') }}" method="POST" class="space-y-4">
                        @csrf
                        <div>
                            <label for="name" class="block text-sm font-medium text-gray-700">Nom</label>
                            <input id="name" name="name" type="text" class="mt-1 p-3 w-full border border-gray-300 rounded-md" value="{{ old('name') }}" required>
                        ...
                        <div class="pt-2 flex space-x-3">
                            ...
                            <a href="{{ route('roles.index') }}" class="px-4 py-2 bg-gray-200 text-gray-800 rounded hover:bg-gray-300">Annuler</a>
                        </div>
                    </form>
                </div>
            </div>
