    ...
    <div class="flex-1 flex flex-col pl-6 pr-4 py-2">
        ...
        <main class="flex-1 p-6 items-center overflow-auto">
            <div class="max-w-xl ml-[250px]">
                <h1 class="text-2xl font-bold mb-6">Modifier le semestre {{ $semestre->code }}</h1>
                <div class="bg-white rounded-lg shadow p-6">
                    <form action="{{ route('semestres.update', $semestre) }}" method="POST" class="space-y-4">
                        ...
                        <div>
                            ...
                            <input id="code" name="code" type="text" class="mt-1 p-3 w-full border border-gray-300 rounded-md" value="{{ old('code', $semestre->code) }}" required>
                        ...
                        <div>
                            ...
                            <select id="annee_id" name="annee_id" class="mt-1 p-3 w-full border border-gray-300 rounded-md" required>
                                @foreach($annees as $a)
                                    <option value="{{ $a->id }}" {{ old('annee_id', $semestre->annee_id) == $a->id ? 'selected' : '' }}>{{ $a->name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <input id="date_debut" name="date_debut" type="date" class="mt-1 p-3 w-full border border-gray-300 rounded-md" value="{{ old('date_debut', optional($semestre->date_debut)->format('Y-m-d')) }}" required>
                        ...
                        <div>
                            ...
                            <input id="date_fin" name="date_fin" type="date" class="mt-1 p-3 w-full border border-gray-300 rounded-md" value="{{ old('date_fin', optional($semestre->date_fin)->format('Y-m-d')) }}" required>
                        ...
                        <div class="pt-2 flex space-x-3">
                            ...
                            <a href="{{ route('semestres.show', $semestre) }}" class="px-4 py-2 bg-gray-200 text-gray-800 rounded hover:bg-gray-300">Annuler</a>
                        </div>
                    </form>
                </div>
            </div>
