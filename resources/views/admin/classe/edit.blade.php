    ...
    <div class="flex-1 flex flex-col pl-6 pr-4 py-2">
        ...
        <main class="flex-1 p-6 items-center overflow-auto">
            <div class="max-w-3xl ml-[250px]">
                <h1 class="text-2xl font-bold mb-6">Modifier la classe {{ $classe->code }}</h1>
                <div class="bg-white rounded-lg shadow p-6">
                    <form action="{{ route('classes.update', $classe) }}" method="POST" class="space-y-4">
                        @csrf
                        @method('PUT')
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label for="code" class="block text-sm font-medium text-gray-700">Code</label>
                                <input id="code" name="code" type="text" class="mt-1 p-3 w-full border border-gray-300 rounded-md" value="{{ old('code', $classe->code) }}" required>
                                @error('code')<p class="text-red-600 text-sm mt-1">{{ $message }}</p>@enderror
                            </div>
                            <div>
                                <label for="nom" class="block text-sm font-medium text-gray-700">Nom</label>
                                <input id="nom" name="nom" type="text" class="mt-1 p-3 w-full border border-gray-300 rounded-md" value="{{ old('nom', $classe->nom) }}" required>
                            ...
                            <div>
                                ...
                                <select id="filiere_id" name="filiere_id" class="mt-1 p-3 w-full border border-gray-300 rounded-md" required>
                                    @foreach($filieres as $f)
                                        <option value="{{ $f->id }}" {{ old('filiere_id', $classe->filiere_id) == $f->id ? 'selected' : '' }}>{{ $f->nom }} ({{ $f->code }})</option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <select id="niveau_id" name="niveau_id" class="mt-1 p-3 w-full border border-gray-300 rounded-md" required>
                                    @foreach($niveaux as $n)
                                        <option value="{{ $n->id }}" {{ old('niveau_id', $classe->niveau_id) == $n->id ? 'selected' : '' }}>{{ $n->code }} - {{ $n->libelle }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <select id="salle_id" name="salle_id" class="mt-1 p-3 w-full border border-gray-300 rounded-md">
                                    <option value="">--</option>
                                    @foreach($salles as $s)
                                        <option value="{{ $s->id }}" {{ old('salle_id', $classe->salle_id) == $s->id ? 'selected' : '' }}>{{ $s->code }} ({{ $s->capacite }})</option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <select id="annee_id" name="annee_id" class="mt-1 p-3 w-full border border-gray-300 rounded-md">
                                    <option value="">--</option>
                                    @foreach($annees as $a)
                                        <option value="{{ $a->id }}" {{ old('annee_id', $classe->annee_id) == $a->id ? 'selected' : '' }}>{{ $a->name }}</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="pt-2 flex space-x-3">
                            ...
                            <a href="{{ route('classes.show', $classe) }}" class="px-4 py-2 bg-gray-200 text-gray-800 rounded hover:bg-gray-300">Annuler</a>
                        </div>
                    </form>
                </div>
            </div>
